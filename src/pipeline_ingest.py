    ...
    try:
        ...
        for table in MITMA_TITLES_RECORDS.keys():
            if table == 'bronze_provincias' or table == 'bronze_festivos':
                local_path = f"data/{MITMA_TITLES_RECORDS[table]}"
                # If local file handling is needed, add here. 
                # For now ignoring as per ingestor.py odd logic or assume handled externally
                pass
            else:
        ...
        # Process found URLs
        for table, url_list in urls.items():
            if url_list:
                ingest_mitma_urls(con, url_list, table)
            else:
                print(f"No URLs found for {table}")
                
        # Process INE
        for table in ine_urls:
            path = ine_urls[table][0]
            if os.path.exists(path):
                ingest_json(con, table, path)
            else:
                print(f"INE file not found: {path}")
